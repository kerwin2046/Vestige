...
from search.search_engine import search_company_footprint, host_of, canonicalize_url
    ...
    *,
    ...
    for lead in inventory:
        ...
        sources.append(
            {
                "url": url,
                "canonical_url": lead.get("canonical_url") or canonicalize_url(url),
                "domain": host_of(url),
...
async def run_company_discovery(
    *,
    ...
    """
    Request-scoped discovery orchestration.

    Returns:
      {
        "result": structured payload from _build_result (or empty sources),
        "report": markdown report string,
        "export_path": optional Excel path string,
      }
    """
    settings = settings or {}
    company = (anchor.get("name") or "").strip()
    if not company:
        raise ValueError("anchor.name is required")

    max_urls = settings.get("max_urls_to_crawl", MAX_URLS_TO_CRAWL)
    if max_urls is None:
        max_urls = MAX_URLS_TO_CRAWL
    ...
    if not inventory:
        empty = _build_result(company, anchor, [], {}, search_backend=backend_name)
        return {
            "result": empty,
            "report": "未能检索到与该公司相关的可信来源。",
            "export_path": None,
        }
    ...
    subset = _select_crawl_subset(inventory, int(max_urls), MAX_CRAWL_PER_DOMAIN)
    print(f"\n🔍 [Step 2] 选取 {len(subset)} 个来源做深度抓取(每域名≤{MAX_CRAWL_PER_DOMAIN})...")
    urls = [r["url"] for r in subset]
    pages = await crawl_pages(urls) if urls else []
    pairs = [(u, p) for u, p in zip(urls, pages) if (p.get("markdown") or "").strip()]
    by_method = Counter(p.get("crawler_method") or "unknown" for _, p in pairs)
    ...
    result = _build_result(
        company, anchor, inventory, enriched, search_backend=backend_name
    )
    export_path = None
    if write_excel:
        excel_path = save_excel(result, output_dir or OUTPUT_DIR)
        export_path = str(excel_path)
        print(f"💾 结构化结果已写入: {excel_path}")

    return {
        "result": result,
        "report": _render_report(company, inventory, enriched),
...
async def run_rag_pipeline(company: str | None = None) -> str:
    """
    CLI entry: company web footprint discovery.
    Uses global COMPANY_ANCHOR; optional company overrides the search name only when
    provided as a full anchor via run_company_discovery.
    """
    anchor = dict(COMPANY_ANCHOR)
    if company:
        anchor["name"] = company
    outcome = await run_company_discovery(anchor=anchor, write_excel=True)
